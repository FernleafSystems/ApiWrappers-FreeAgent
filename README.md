...
implementation. It uses the their latest version and make this package compatible with PHP 5.6+
...
the accessor methods you need. You should store the token for future use and the expiration time
of the token, and also the refresh token - you use this once the token has expired to
request a new token
...
and feed this into the API entity Class that we want to use. In this case it's a retrieval
...
Note: If you don't set the Sandbox flag, it'll assume it's **not** operating in the sandbox
...
The `setEntityId()` is the primary method for retrieving and updating entites. You must
know the ID before you can attempt to retrieve it or update it, of course.

If you don't provide the ID, the system will throw an Exception before even making the request.

The object returned, in this particular case, will be a `ContactVO`. It's similar to the
`CompanyVO` provided in the previous example, along with helpful accessor methods so you 
don't need to go looking up keys in the API docs.
...
present the API will throw an Exception before even making a request.  You can find out
what is critical by looking at the method `getCriticalRequestItems()` on each API entity.

What if you want to find something, because you don't know the ID?
...
created, so this could be quite a big API operation while it tries to find your bill.

Instead, you could limit it by date, and even us a range with a radius.
...
parameter must be a UNIX timestamp, and the 2nd parameter is the number of days radius
outside of the timestamp, so in this case, 5 days before, and 5 days after - a time
window of 10 days total.
...
```php
	$aAllBills = ( new \FernleafSystems\ApiWrappers\Freeagent\Entities\Bills\Find() )
	    ...
	    ->all();
```
We've removed the Reference filter and now calling using `all()`. This will return us
an array of `BillVO` objects. It may be empty, or not.

## The Rest...

Each entity type has its own set of methods and parameter, and filter options. You have
to look at each to determine what your options are.

The examples above to specific to those particular entites in question. There is of course
some overlap, but it follows the documentation and specification of the FreeAgent API.
